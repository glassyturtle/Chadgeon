...
public class LobbyUI : MonoBehaviour
{
    ...
    [SerializeField] private GameObject[] expirimentalMapButtons;
    ...
    private void UpdateLobby(Lobby lobby)
    {
        ...
        else
        {
            if (MultiplayerManager.Instance.IsLobbyHost())
            {
                ...
                if (GameDataHolder.expirimentalMode)
                {
                    foreach (GameObject obj in expirimentalMapButtons)
                    {
                        obj.SetActive(true);
                    }
                }
    ...
    IEnumerator DelayUpdate()
    {
        ...
        string diffName = "Mixed";
        if (botDifficulty != -1)
        {
            diffName = botDifficultyNames[botDifficulty];
        }
        if (selectedMap == -1)
        {
            MultiplayerManager.Instance.UpdateLobbySettings("Random", diffName, amtOfFlocks.ToString());

        }
        else
        {
            MultiplayerManager.Instance.UpdateLobbySettings(mapNames[GameDataHolder.map], diffName, amtOfFlocks.ToString());
