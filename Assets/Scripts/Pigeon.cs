...
public class Pigeon : NetworkBehaviour
{
    ...
    [SerializeField] private SpriteRenderer allyIconMinimap;
    ...
    public struct DealtDamageProperties : INetworkSerializable
    {
        ...
        public void NetworkSerialize<T>(BufferSerializer<T> serializer) where T : IReaderWriter
        {
            ...
            serializer.SerializeValue(ref wasPatrickBateman);
        }
    }
    #endregion
    ...
    public void OnPigeonHitCLientRPC(AttackProperties atkProp)
    {
        ...
        else
        {
            ...
            HandleKnockbackServerRpc(totalKnockback * (totalDamageTaking / 2) * knockbackMod * direction);
        }
        ...
        if (hasBeenKO && pigeonAI && pigeonName == "Patrick Bateman")
        {
            ddProp.wasPatrickBateman = true;
        }
        ...
        if (!isPlayer && GameDataHolder.gameMode != "Supremacy")
        {
            ...
            {
                case 0:
                    ddProp.xpOnKill = Mathf.RoundToInt(ddProp.xpOnKill * 0.6f);
                    break;
                case 1:
                    ddProp.xpOnKill = Mathf.RoundToInt(ddProp.xpOnKill * 0.5f);
                    break;
                case 2:
                    ddProp.xpOnKill = Mathf.RoundToInt(ddProp.xpOnKill * 0.4f);
                    break;
                case 3:
                    ddProp.xpOnKill = Mathf.RoundToInt(ddProp.xpOnKill * 0.3f);
                    break;
            }
        }
        */
    ...
    public void ReciveDamageClientRpc(DealtDamageProperties ddProp, ulong pigeonID)
    {
        ...
        if (ddProp.hasDied)
        {
            if (isPlayer)
            {
                ...
                if (ddProp.wasPatrickBateman) SteamIntegration.instance.UnlockAchivement("what_sigma?");
            }
    ...
    public void AddUpgrade(Upgrades upgrade)
    {
        ...
        switch (upgrade)
        {
            ...
            case Upgrades.razorFeathers:
                ...
                hasAbilityM2 = true; break;
            case Upgrades.overclock:
                if (pigeonUpgrades.ContainsKey(Upgrades.razorFeathers))
                {
                    chargedFeathers = 3;
                    m2AbilityCooldown = 0;
                    if (isPlayer) GameManager.instance.StartCooldown(Upgrades.razorFeathers, 0);
                }
                if (pigeonUpgrades.ContainsKey(Upgrades.slam))
                {
                    m2AbilityCooldown = 0;
                    if (isPlayer) GameManager.instance.StartCooldown(Upgrades.slam, 0);
                }
                if (pigeonUpgrades.ContainsKey(Upgrades.mewing))
                {
                    qAbilityCooldown = 0;
                    if (isPlayer) GameManager.instance.StartCooldown(Upgrades.mewing, 0);
                }
                if (pigeonUpgrades.ContainsKey(Upgrades.wholeGains))
                {
                    eAbilityCooldown = 0;
                    if (isPlayer) GameManager.instance.StartCooldown(Upgrades.wholeGains, 0);
                }
                if (pigeonUpgrades.ContainsKey(Upgrades.pigeonPoo))
                {
                    eAbilityCooldown = 0;
                    if (isPlayer) GameManager.instance.StartCooldown(Upgrades.pigeonPoo, 0);
                }
                if (pigeonUpgrades.ContainsKey(Upgrades.hiddinTalon))
                {
                    qAbilityCooldown = 0;
        ...
        if (flock != 0)
        {
            ...
            switch (flock)
            {
                case 1:
                    ...
                    if (GameManager.instance.player.flock == flock && GameManager.instance.player != this)
                    {
                        allyIconMinimap.gameObject.SetActive(true);
                        allyIconMinimap.color = Color.cyan;
                    }
                    break;
                case 2:
                    ...
                    {
                        allyIconMinimap.gameObject.SetActive(true);
                        allyIconMinimap.color = Color.red;
                    }
                    break;
                case 3:
                    ...
                    {
                        allyIconMinimap.gameObject.SetActive(true);
                        allyIconMinimap.color = Color.yellow;
                    }
                    break;
                case 4:
                    ...
                    {
                        allyIconMinimap.gameObject.SetActive(true);
                        allyIconMinimap.color = Color.green;
                    }
                    break;
            }
    ...
    protected void PigeonThrow(AttackProperties atkProp, Quaternion theAngle)
    {
        ...
        if (pigeonUpgrades.TryGetValue(Upgrades.critcalDamage, out bool _) && Random.Range(0, 100) <= 25)
        {
            atkProp.damage *= 2;
            if (isPlayer && isMaxing && pigeonUpgrades.TryGetValue(Upgrades.brawler, out _))
            {
    ...
    protected void LandAssassinate()
    {
        ...
        Vector2 pos = transform.position;
        pos = Vector2.MoveTowards(pos, targetPigeon.transform.position, 0.1f);
